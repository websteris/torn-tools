...
async function processSpecificWarData(factionId, warId, warType, warData, timestamp = null) {
  try {
    ...
    // Different war types have different data structures
    if (warType === 'ranked') {
      // Ranked wars have a nested structure with factions and war details
      ...
      // Store the main war record
      await storeWarData(warRecord);
      
      // Process each faction in the war
      ...
      // Territory and raid wars typically have assaulting/defending factions
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      `, [warId, warType], async (err, warRow) => {
        ...
        // For ranked wars, get the faction data
        if (warType === 'ranked') {
          db.all(`
          ...
          `, [warId], (err, factionsRows) => {
            ...
            // Combine war data with faction data

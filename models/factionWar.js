...
async function getFactionWarById(war_id) {
  try {
    const war = await db('faction_wars').where({ war_id }).first();
    ...
    logger.error(`Error getting faction war by ID ${war_id}: ${error.message}`);
    throw error;
  }
}

async function getFactionWarByOpponent(faction_id, opponent_id) {
  try {
    const war = await db('faction_wars')
      .where({ faction_id, opponent_id })
      .first();
    ...
    logger.error(`Error getting faction war between ${faction_id} and ${opponent_id}: ${error.message}`);
    throw error;
  }
}

async function getActiveWarsByFactionId(faction_id) {
  try {
    const wars = await db('faction_wars')
      .where({ faction_id, status: 'active' })
      .select('*');
